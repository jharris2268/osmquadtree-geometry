        ...
        match prfx {
            Some(prfx) => {
                serde_json::to_writer(
                    std::fs::File::create(&format!("{}spec.json", prfx)).expect("!"),
                    &CopySpec::new(tabs, tabs.len() > 3),
                )
                .expect("!");
            }
            None => {}
                ...
                match self.outputs.get_mut(&pb.table) {
                    Some(pp) => {
                        ...
                        match pp.1.as_mut() {
                            None => {}
                            Some(q) => {
                                q.call(std::mem::take(&mut pb.data));
                            }
                        }
                    }
                    None => {
...
fn prep_output(
    ...
    match &opts.connection {
        ...
        PostgresqlConnection::CopyFilePrfx(p) => Ok(Box::new(WritePackedFiles::new(
            Some(&p),
            &opts.table_spec,
            numchan != 0,
        ))),
        PostgresqlConnection::CopyFileBlob(p) => {
            Ok(make_write_packed_pbffile(&p, &opts.table_spec, numchan))
        }
    }
}
    ...
    T: CallFinish<CallType = Vec<PackedBlob>, ReturnType = Timings> + ?Sized,
{
    ...
    fn call_all(&mut self, geoms: &GeometryBlock) -> Vec<PackedBlob> {
        ...
        let (c, e) = pack_geometry_block(&self.preptables, &mut blobs, &*self.alloc_func, &geoms)
            .expect("!");
        self.count += c;
        self.errs += e;
        blobs
...
pub fn make_write_postgresql_geometry(
    options: &PostgresqlOptions,
    numchan: usize,
) -> Result<crate::CallFinishGeometryBlock> {
    ...
    if numchan == 0 {
        Ok(Box::new(PackPostgresData::new(
            out,
            options.table_alloc.clone(),
            preptables.clone(),
        )))
    } else {
        let outs = CallbackSync::new(out, numchan);
        let mut packs: Vec<crate::CallFinishGeometryBlock> = Vec::new();
        for o in outs {
            let o2 = Box::new(ReplaceNoneWithTimings::new(o));
            packs.push(Box::new(Callback::new(Box::new(PackPostgresData::new(
                o2,
                options.table_alloc.clone(),
                preptables.clone(),
            )))));
        }
        Ok(Box::new(CallbackMerge::new(
            packs,

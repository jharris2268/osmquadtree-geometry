    ...
    T: CallFinish<CallType = WorkingBlock, ReturnType = Timings, ErrorType=Error> + ?Sized,
{
    ...
    fn process_block(&mut self, bl: &mut WorkingBlock) -> Option<WorkingBlock> {
        ...
        if !tt.is_empty() {
            self.pending_tiles
                .insert(bl.geometry_block.quadtree.clone(), tt);
        }
        bl.pending_nodes = nn;
    ...
    fn finish(&mut self) -> ccResult<Timings, Error> {
        ...
        tms.add_other(
            ...
            OtherData::Messages(vec![format!(
                "{} nodes kept, {} with tags added, {} finished",
                self.total_added, self.tags_added, self.finished
            )]),
        );
        Ok(tms)
    }
